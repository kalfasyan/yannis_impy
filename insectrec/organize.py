...
if year == '2018':
	...
	try:
		DATADIR = results_dir(year, week_nr=sys.argv[1], base_dir=BASE_DATA_DIR) # given a week number, select the data
	...
	data_dirs = results_dir(year, base_dir=BASE_DATA_DIR) # given a week number, select the data
...
platenames = pd.Series(plates).apply(lambda x: x.split("/")[-1][:-4])
...
df_stats = pd.DataFrame(columns=['nr_nans','unique_insects','annotated'], index=platenames)
all_specs = []

path_annotations = 'created_data/annotations/'
path_images = 'created_data/images/'
...
for p, platename in tqdm(enumerate(plates)):
	...
	if condition1 and condition2 and condition3:
		print(f'{condition1 and condition2} ----> COPYING IT')
		annotated_plates.append(platename)
		print(platename)

### TODO: MAKE EXTRACTED BOXES SQUARE TO AVOID RESIZING ISSUES LATER
